<template>
	<section>
		...
		<el-col :span="24" class="toolbar">
			...
			<el-pagination layout="prev, pager, next" @current-change="handleCurrentChange" :page-size="20" :total=total*2 style="float:right;">
			</el-pagination>
		</el-col>
		...
		<el-dialog title="编辑" v-model="editFormVisible" :close-on-click-modal="false">
			...
			<el-form :model="editForm" label-width="80px" :rules="editFormRules" ref="editForm">
				<el-form-item label="系统编号" prop="id">
					<el-input disabled v-model="editForm.id" auto-complete="off"></el-input>
				</el-form-item>
...
<script>
	...
	import { spotlight,DeleteUserById,updateuser } from "@/graphql/search";
	import {login,register,checkLoginFormValidity,checkRegisterFormValidity} from "@/graphql/admin";
	export default {
		name: "用户信息管理",
		data() {
			return {
		...
		methods: {
		  ...
		  //console.log(usersResponse.data.Users.numOfPages * this.perPage);
		  for(let i=0;i<this.users.length;i++){
			  if(this.users[i].role===true){
				  this.users[i].role="1"
			  }else{
				  this.users[i].role="0"
			  }
		  }
		  this.total = usersResponse.data.Users.numOfPages * this.perPage;
